    ...
    UUPSUpgradeable
{
    ...
    /**
     * @notice Dev function to manually set the expiration for a stake ID
     * @dev Reverts if the Stake ID does not exist
     * @param stakeId The ID of the Stake to set the expiration
     * @param expiration The new expiration
     */
    function setExpiration(uint256 stakeId, uint256 expiration) external {
        // It also reverts if stakeId does not exist
        if (msg.sender != ownerOf(stakeId)) {
            revert InvalidStakeId(stakeId);
        }
        ...
        DimoStakingStorage storage $ = _getDimoStakingStorage();
        StakingData memory stakingData = $.stakeIdToStakingData[stakeId];
        address stakingTo = $.stakerToStake[msg.sender];

        $.stakeIdToStakingData[stakeId].lockEndTime = expiration;

        uint256 points = $.stakingLevels[stakingData.level].points;
        emit Staked(msg.sender, stakeId, stakingTo, stakingData.level, stakingData.amount, expiration, points);
    }
