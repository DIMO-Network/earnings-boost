...
const ProxyTestModule = buildModule('ProxyTestModule', (m) => {
    ...
    const implementation = m.contract('DIMOStakingDev')
...
const DIMOStakingDevTestModule = buildModule('DIMOStakingTestModule', (m) => {
    ...
    const dimoStaking = m.contractAt('DIMOStakingDev', proxy)

    return { dimoStaking, proxy, mockDimoToken, mockVehicleId }
})

export default DIMOStakingDevTestModule
